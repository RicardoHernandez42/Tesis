...
for i in range(0,len(MiQSO)):
    ...
    if MiQSO[i] != -9.9:
        d=str(int(ID[i]))
        if ('/Volumes/ADATA/Pruebatxt/LC_'+d+'_completo_promedio.txt') in '/Volumes/ADATA/Pruebatxt/':
            shutil.move('/Volumes/ADATA/Pruebatxt/LC_'+d+'_completo_promedio.txt', '/Volumes/ADATA/MiQSO/')
