...
mypath = '/Volumes/ADATA/Entrenamientotxt2/'
carpeta = '/Volumes/ADATA/Entrenamientotxt2'
onlyfiles = [f for f in listdir(mypath) if isfile(join(mypath, f))]
count = 1
for c in onlyfiles:
    file = os.path.splitext(c)[0]
    with open('/Volumes/ADATA/Entrenamiento2 9000QSO completo/LC_'+file+'_completo.txt', 'w') as out:
        for i in range(1,31):
            out.write('0,0,0,0,0' + '\n')
        print(count,c)
        count=count+1
        ufecha,u1,uerr,gfecha,g1,gerr,rfecha,r1,rerr,ifecha,i1,ierr,zfecha,z1,zerr,Ra,Dec, = np.loadtxt(mypath + '/' + c, delimiter=' ',unpack=True)
        r = []
        i = []
        u = []
        z = []
