...
mypath = '/Volumes/ADATA/RRLyraetxtNuevostxtcompletos/'
carpeta = '/Volumes/ADATA/RRLyraetxtNuevostxtcompletos/'
onlyfiles = [f for f in listdir(mypath) if isfile(join(mypath, f))]
for c in onlyfiles:
    file = os.path.splitext(c)[0]
    with open('/Volumes/ADATA/RRLyraetxtNuevostxtpromedio/'+file+'_promedio.txt', 'w') as out:
        h = open(mypath + '/' + c)
        text2 = h.readlines()
        h.close()
        print(c)
